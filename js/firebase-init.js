...
async function inicializar() {
  if (FIREBASE_ENABLED) {
    ...
    auth = getAuth(app);
    db = getFirestore(app);
    // Persistencia explícita: la sesión se guarda en el dispositivo y sobrevive
    // cerrar la app o el navegador, así el usuario no tiene que iniciar sesión
    // cada vez que entra. Esperamos a que se aplique antes de continuar para
    // evitar que un login ocurra antes de que la persistencia esté configurada.
    try {
      await setPersistence(auth, browserLocalPersistence);
    } catch (e) { /* si falla, Firebase usa su persistencia por defecto */ }
  }
