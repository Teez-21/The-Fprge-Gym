...
const TabEjercicios = (() => {
  ...
  async function render() {
    ...
    panel.innerHTML = `
      ...
      <p class="section-subtitle">Compartida con todos los usuarios de The Forge. Lo que agregues o edites, todos lo ven.</p>
    ...
    // Tiempo real: si un compañero agrega/edita un ejercicio, se refleja solo
    if (state.unsubscribe) state.unsubscribe();
    state.unsubscribe = ForgeStorage.escucharEjerciciosGlobales((lista) => {
      state.ejercicios = lista;
      if (document.getElementById("exerciseList")) renderList();
    });
  }
  ...
  function abrirFormularioEjercicio(ejercicioExistente) {
    ...
    ForgeUI.modal({
      ...
      bodyHTML: `
        ...
        <p style="font-size:11px;color:var(--text-muted);">Este ejercicio quedará visible para todos los usuarios de The Forge.</p>
      `,
      buttons: [
        ...
        { label: esEdicion ? "Guardar cambios" : "Crear ejercicio", variant: "primary", onClick: async () => {
          ...
          await ForgeStorage.guardarEjercicioGlobal(nuevoEjercicio);
          ...
          ForgeUI.toast(esEdicion ? "Ejercicio actualizado para todos." : "Ejercicio agregado para todos.");
        }}
      ]
    });
  }
