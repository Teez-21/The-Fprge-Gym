...
const TabRutina = (() => {
  ...
  async function revisarAprobacionesResueltas() {
    const perfil = await ForgeStorage.getPerfil();
    if (!perfil?.grupoId) return;
    const grupo = await ForgeStorage.getGrupo(perfil.grupoId);
    if (!grupo) return;

    const miUid = ForgeStorage.getUid();
    const misResueltas = (grupo.solicitudesAprobacion || []).filter(
      s => s.uid === miUid && s.estado !== "pendiente" && !s.procesadoLocalmente
    );

    for (const s of misResueltas) {
      if (s.estado === "aprobado") {
        await aplicarPoderDeEntrenoAprobado(s.entrenoId);
      }
      // Marcamos como procesada localmente para no reaplicar poder dos veces.
      // Como solo el dueño del uid puede escribir en su perfil/entrenos, esto
      // es seguro: cada quien resuelve sus propias solicitudes resueltas.
      const solicitudesActualizadas = grupo.solicitudesAprobacion.map(sol =>
        sol.id === s.id ? { ...sol, procesadoLocalmente: true } : sol
      );
      await ForgeStorage.actualizarGrupo(perfil.grupoId, { solicitudesAprobacion: solicitudesActualizadas });
    }
  }

  async function aplicarPoderDeEntrenoAprobado(entrenoId) {
    const historial = await ForgeStorage.getHistorialEntrenos(200);
    const entreno = historial.find(e => e.id === entrenoId);
    if (!entreno || entreno._poderAplicado) return;

    // Reconstruimos el historial previo a este entreno (excluyéndolo) para el cálculo de bonus de progreso
    const historialPrevio = {};
    historial.filter(e => e.id !== entrenoId && new Date(e.fecha) < new Date(entreno.fecha))
      .slice().reverse()
      .forEach(e => (e.ejercicios || []).forEach(ej => {
        historialPrevio[ej.exerciseId] = { series: ej.series, fecha: e.fecha, nombre: ej.nombre };
      }));

    const { total, desglose } = calcularPoderEntreno(entreno, historialPrevio);

    // También cuenta para la racha del grupo, ya que estaba pendiente por ubicación.
    if (window.TabGrupo) {
      await window.TabGrupo.registrarActividadHoy();
    }

    const resultado = await ForgeStorage.sumarPoder(total);
    await ForgeStorage.actualizarEntreno(ForgeStorage.getUid(), entrenoId, { _poderAplicado: true, estadoVerificacion: "aprobado" });

    ForgeUI.modal({
      title: "✅ Entreno aprobado",
      bodyHTML: `<p>Tu compañero confirmó tu entreno. Se sumaron <strong>${total} de Poder</strong> a tu Legado.</p>`,
      buttons: [{ label: "Genial", variant: "forge", onClick: () => {
  ...
  function renderExerciseCard(ej, exIdx) {
    ...
    card.innerHTML = `
      ...
      <div data-series-summary="${exIdx}"></div>
      <button class="add-series-btn" data-add-series="${exIdx}">+ Agregar bloque de series</button>
    `;

    card.querySelector(`[data-add-series="${exIdx}"]`).onclick = () => abrirBloqueSeries(ej, card, exIdx);
    card.querySelector(".exercise-card__remove").onclick = () => {
      state.ejerciciosDelDia.splice(exIdx, 1);
      renderEjercicios();
    };

    renderSeriesSummary(card, ej, exIdx);
    return card;
  }

  /** Resumen visual de las series ya registradas, agrupadas por bloque idéntico */
  function renderSeriesSummary(card, ej, exIdx) {
    const wrap = card.querySelector(`[data-series-summary="${exIdx}"]`);
    if (!ej.series.length) {
      wrap.innerHTML = `<p class="text-muted" style="font-size:12px;margin:8px 0;">Sin series registradas todavía.</p>`;
      return;
    }
    wrap.innerHTML = `
      <table class="series-table">
        <thead><tr><th>#</th><th>Kg</th><th>Reps</th><th></th></tr></thead>
        <tbody>
          ${ej.series.map((s, sIdx) => `
            <tr>
              <td>${sIdx + 1}</td>
              <td>${ForgeNumberPicker.formatWeightEs(s.peso || 0)}</td>
              <td>${s.reps || 0}</td>
              <td><button class="series-row__delete" data-del-serie="${sIdx}">×</button></td>
            </tr>
          `).join("")}
        </tbody>
      </table>
    `;
    wrap.querySelectorAll("[data-del-serie]").forEach(btn => {
      btn.onclick = () => {
        ej.series.splice(parseInt(btn.dataset.delSerie), 1);
        renderSeriesSummary(card, ej, exIdx);
      };
    });
  }

  /** Modal para agregar un bloque de N series iguales: cantidad, peso, reps (con pickers giratorios) */
  function abrirBloqueSeries(ej, card, exIdx) {
    ForgeUI.modal({
      title: "Agregar series",
      bodyHTML: `
        <div style="display:flex;gap:10px;">
          <div class="picker-group" style="flex:1;">
            <span class="picker-group__label">Series</span>
            <div class="number-picker-frame" id="pickerSeriesWrap"></div>
          </div>
          <div class="picker-group" style="flex:1.3;">
            <span class="picker-group__label">Kg</span>
            <div class="number-picker-frame" id="pickerPesoWrap"></div>
          </div>
          <div class="picker-group" style="flex:1;">
            <span class="picker-group__label">Reps</span>
            <div class="number-picker-frame" id="pickerRepsWrap"></div>
          </div>
        </div>
      `,
      buttons: [
        { label: "Cancelar", variant: "secondary" },
        { label: "Agregar", variant: "primary", onClick: () => {
          for (let i = 0; i < bloqueState.series; i++) {
            ej.series.push({ peso: bloqueState.peso, reps: bloqueState.reps });
          }
          renderSeriesSummary(card, ej, exIdx);
    ...
    const bloqueState = { series: 3, peso: 0, reps: 8 };

    const pickerSeries = ForgeNumberPicker.createIntPicker({
      min: 1, max: 10, value: bloqueState.series,
      onChange: (v) => bloqueState.series = v
    });
    const pickerPeso = ForgeNumberPicker.createWeightPicker({
      min: 0, max: 400, step: 2.5, value: bloqueState.peso,
      onChange: (v) => bloqueState.peso = v
    });
    const pickerReps = ForgeNumberPicker.createIntPicker({
      min: 1, max: 50, value: bloqueState.reps,
      onChange: (v) => bloqueState.reps = v
    });

    document.getElementById("pickerSeriesWrap").appendChild(pickerSeries);
    document.getElementById("pickerPesoWrap").appendChild(pickerPeso);
    document.getElementById("pickerRepsWrap").appendChild(pickerReps);
  }

  function evaluarPopupRepsSiAplica(ej) {
    if (ej.series.length < 2) return;
    const primeras2 = ej.series.slice(0, 2);
    if (primeras2.some(s => !s.reps)) return;
    if (ej._avisoMostrado) return;
    const resultado = evaluarPrimerasSeries(primeras2);
    if (resultado === "bajar") {
  ...
  function abrirCapturaFoto() {
    ForgeUI.modal({
      title: "📸 Verifica tu entreno",
      bodyHTML: `
        <p style="margin-bottom:14px;">Toma o sube una foto del gimnasio, equipo o lugar donde entrenaste.</p>
        <input type="file" accept="image/*" capture="environment" id="fotoEntrenoInput" style="margin-bottom:10px;width:100%;">
        <div id="fotoPreviewWrap"></div>
      `,
      buttons: [
        { label: "Cancelar", variant: "secondary" },
        { label: "Confirmar entreno", variant: "primary", onClick: async () => {
          const input = document.getElementById("fotoEntrenoInput");
          if (!input.files || !input.files[0]) {
            ForgeUI.toast("Toma o selecciona una foto para continuar.");
            // Reabrir el modal de captura porque el cierre ya fue disparado
            setTimeout(abrirCapturaFoto, 350);
            return;
          }
          const fotoDataUrl = await ForgeVerification.comprimirImagen(input.files[0]);
          procesarEntrenoConFoto(fotoDataUrl);
        }}
      ]
    });
  }

  async function procesarEntrenoConFoto(fotoDataUrl) {
    ForgeUI.toast("Verificando ubicación...", 1500);

    const ubicacionActual = await ForgeVerification.obtenerUbicacionActual();
    const gimnasioRef = await ForgeStorage.getGimnasioReferencia();
    const evalUbicacion = ForgeVerification.evaluarUbicacion(gimnasioRef, ubicacionActual);

    // Si es el primer entreno del usuario (sin gimnasio de referencia aún),
    // lo guardamos como referencia silenciosamente, sin fricción.
    if (evalUbicacion.esPrimeraVez && ubicacionActual) {
      await ForgeStorage.guardarGimnasioReferencia(ubicacionActual.lat, ubicacionActual.lon);
    }

    const requiereAprobacion = evalUbicacion.dentroDelRadio === false;
    await guardarYProcesarEntreno(fotoDataUrl, requiereAprobacion);
  }

  // ---------------- FINALIZAR ENTRENO ----------------
  async function guardarYProcesarEntreno(fotoDataUrl, requiereAprobacion) {
    ...
    const entreno = {
      ...
      esRegreso,
      fotoDataUrl,
      estadoVerificacion: requiereAprobacion ? "pendiente" : "aprobado"
    };

    const entrenoId = await ForgeStorage.guardarEntreno(entreno);

    // Avanzar al siguiente día de la rotación (esto siempre avanza, sin importar la aprobación)
    ...
    if (requiereAprobacion) {
      await manejarEntrenoPendiente(entreno, entrenoId, fotoDataUrl);
    } else {
      await sumarPoderYNotificar(entreno, historialPrevio);
    }
  ...
  async function manejarEntrenoPendiente(entreno, entrenoId, fotoDataUrl) {
    const perfil = await ForgeStorage.getPerfil();
    if (!perfil?.grupoId) {
      // Sin grupo, no hay quién apruebe: lo dejamos pendiente informativamente,
      // pero no bloqueamos al usuario solitario indefinidamente.
      ForgeUI.modal({
        title: "📍 Ubicación distinta a la habitual",
        bodyHTML: `<p>Este entreno se registró desde un lugar distinto a tu gimnasio habitual. Como no tienes un grupo que lo apruebe, quedará registrado pero no sumará a tu Poder. Si esto es un error, puedes seguir entrenando ahí y se actualizará tu referencia.</p>`,
        buttons: [{ label: "Entendido", variant: "primary" }]
      });
      return;
    }

    await ForgeStorage.crearSolicitudAprobacion(perfil.grupoId, {
      entrenoId,
      fotoDataUrl,
      motivo: "ubicación distinta al gimnasio habitual"
    });

    ForgeUI.modal({
      title: "📍 Necesitamos confirmación",
      bodyHTML: `<p>Este entreno se registró desde un lugar distinto a tu gimnasio habitual. Le pedimos a tu compañero de grupo que lo confirme — en cuanto lo apruebe, sumará a tu racha y Poder.</p>`,
      buttons: [{ label: "Entendido", variant: "primary" }]
    });
  }

  async function sumarPoderYNotificar(entreno, historialPrevio) {
  ...
  return { render, aplicarPoderDeEntrenoAprobado };
})();
