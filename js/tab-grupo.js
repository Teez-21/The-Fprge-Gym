...
const TabGrupo = (() => {
  ...
  async function render() {
    ...
    state.unsubscribe = ForgeStorage.escucharGrupo(state.perfil.grupoId, (grupoActualizado) => {
      ...
      if (document.getElementById("aprobacionesWrap")) renderAprobaciones();
  ...
  function renderAprobaciones() {
    const wrap = document.getElementById("aprobacionesWrap");
    if (!wrap) return;
    const miUid = ForgeStorage.getUid();
    const pendientes = (state.grupo.solicitudesAprobacion || []).filter(s => s.estado === "pendiente");

    if (pendientes.length === 0) { wrap.innerHTML = ""; return; }

    wrap.innerHTML = pendientes.map(s => {
      const esMia = s.uid === miUid;
      const nombreSolicitante = state.grupo.nombres?.[s.uid] || "Compañero";
      return `
        <div class="card" style="border-color:var(--color-forge-orange);">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <span style="font-size:13px;font-weight:600;">📸 ${esMia ? "Tu entreno necesita aprobación" : `${nombreSolicitante} necesita tu aprobación`}</span>
          </div>
          ${s.fotoDataUrl ? `<img src="${s.fotoDataUrl}" style="width:100%;border-radius:var(--radius-sm);margin-bottom:8px;" alt="Foto de entreno">` : ""}
          <p class="text-muted" style="font-size:11px;margin-bottom:10px;">Motivo: ${s.motivo}</p>
          ${esMia ? `
            <p class="text-muted" style="font-size:12px;">Esperando que tu compañero confirme...</p>
          ` : `
            <div style="display:flex;gap:8px;">
              <button class="btn btn--secondary btn--sm" style="flex:1;" data-rechazar="${s.id}">Rechazar</button>
              <button class="btn btn--forge btn--sm" style="flex:1;" data-aprobar="${s.id}">Aprobar</button>
            </div>
          `}
        </div>
      `;
    ...
    wrap.querySelectorAll("[data-aprobar]").forEach(btn => {
      btn.onclick = () => resolverAprobacion(btn.dataset.aprobar, true);
    });
    wrap.querySelectorAll("[data-rechazar]").forEach(btn => {
      btn.onclick = () => resolverAprobacion(btn.dataset.rechazar, false);
    });
  }

  async function resolverAprobacion(solicitudId, aprobado) {
    await ForgeStorage.resolverSolicitudAprobacion(state.grupo.id, solicitudId, aprobado);
    ForgeUI.toast(aprobado ? "Entreno aprobado." : "Entreno rechazado.");
    // Nota: el dueño del entreno es quien procesa el Poder/estado cuando vuelva
    // a abrir su app (revisarAprobacionesResueltas en tab-rutina.js), porque
    // las reglas de seguridad solo permiten que cada usuario escriba en su
    // propio documento — no podemos modificarlo desde aquí.
